local c = require('misery.colors')
local cfg = vim.g.misery_config
local util = require("misery.util")
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('misery.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "misery.nvim" })
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.misery_config.highlights) do
